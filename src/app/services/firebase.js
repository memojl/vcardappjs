...
export default function firebaseService() {
  ...
  $('#app-modulo').on('click', '.btn-add', function () {
    ...
    idVcard.on('value', function (datos) {
      ...
      $('#ID').val(ID);
      let orden = parseInt(ureg.orden) - 1;
      console.log(orden);
      $('#orden').val(orden);
    });
  ...
  function vuser(uidUser) {
    ...
    refUser.on('value', function (datos) {
      ...
      if (reg == null) {
        vsignup(uidUser);
        console.log('vsignup(Activo)+null');
      }
      // Recorremos los productos y los mostramos
      $.each(reg, function (indice, valor) { //console.log(indice);
        const {
        ...
        if (uidUser == uid) {
          datos_user(valor); //console.log('vuser');console.log(reg);
  ...
  function vsignup(uidUser) {
    ...
    refSignup.on('child_added', function (datos) {
      ...
      if (uidUser == uid) {
        datos_user(reg); //console.log('vsignup');console.log(reg);
        var photo = (foto != '' && foto != null && foto != 'undefined') ? foto : page_url + 'bloques/files/images/photos/sinfoto.png';
        var nombre = (usuario != '' && usuario != null && usuario != 'undefined') ? usuario : 'Sin Nombre';
  ...
  $('#app-modulo').on('click', '.btnEditar3', function () {
    ...
    if (Id != '') {
      ...
      console.log(fc);
      $('#f_create').val(fc);
      const direc = document.querySelector('#direc1').textContent;
      console.log(direc);
      $('#direc').val(direc);
      const tel = document.querySelector('#tel1').textContent;
      console.log(tel);
      $('#tel').val(tel);
      const level = document.querySelector('#level1').textContent;
      console.log(level);
      $('#level').val(level);
      const tipoc = document.querySelector('#tipoc1').textContent;
      console.log(tipoc);
      $('#tipoc').val(tipoc);
      const codi = document.querySelector('#codi1').textContent;
      console.log(codi);
      $('#codi').val(codi);
    }
